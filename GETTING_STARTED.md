...
- Ansible - type `ansible --version` to check you have it or install it (docs [here](https://docs.ansible.com/projects/ansible/latest/installation_guide/intro_installation.html)), you will need python3 as well.
- OpenTofu - type (`tofu --version`) to check you have it or install it (docs [here](https://opentofu.org/docs/intro/install/)).
...
- OS: Proxmox VE (preferablly newest for best security) on older version may arrise some errors but it wasn't checked. The newest version official guide installation is available [here](https://www.proxmox.com/en/products/proxmox-virtual-environment/get-started).
...
Go to [INSTALLATION_STEPS](INSTALLATION_STEPS.md) where runbooks are listed and explained.
